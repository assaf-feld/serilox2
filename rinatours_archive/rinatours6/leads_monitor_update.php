...
if ($session[status] == "update")
{
	...
	if ($start_date_now) { $sth = $dbh->query("select current_date"); $start_date_now = $sth->fetch(); $start_date_now = $start_date_now[current_date]; }
	if ($end_date_now) { $sth = $dbh->query("select current_date"); $end_date_now = $sth->fetch(); $end_date_now = $end_date_now[current_date]; }

	if($_POST[update_default])
	{
		$dbh->exec("delete from leads_query where usercode = '$session[usercode]'");
		$dbh->exec("insert into leads_query values(NULL, '$date_type', '$start_date', '$end_date', '$media_1', '$media_2', '$interested_in', '$sales_rep', '$session[usercode]', '$as_result', '$start_date_now', '$end_date_now', 
		$as_main_results','$why_not_reason')");
		print <<<ENDLINE
...
if ($_POST[show_results])
{
	...
	<tr><td>Customer</td><td>Media Main</td><td>Media Secondary</td><td>Scheduled On</td><td>Scheduled For</td><td>Interested In</td><td>Lead Type</td><td>Status</td><td>Summary</td><td>Not Interested Reason<td>&nbsp;</td></tr>
ENDLINE;
