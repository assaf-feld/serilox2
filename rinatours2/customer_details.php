...
function open_contact_list()
{
	...
	{
		document.new_lead_iframe.location = "contacts_list.php?session_index=$session[session_index]&customer_code=" + document.forms[0].customer_code.value + "&ic=$ic";
		$("#customer_details_new_lead").fadeIn(300);
	}
}
...
<div align="center">
<h1>Customer View</h1>
<div style="width: 1200px;"><div class="customer_details_title">General Info</div></div><p>&nbsp;</p>
...
<td><select name="customer_status" class="customer_details_text"><option value=""></option>
ENDLINE;
print "<option value='active'"; if ($customer[status] == "active") { print " selected"; } print ">Active</option>";
print "<option value='not_active'"; if ($customer[status] == "not_active") { print " selected"; } print ">Not Active</option>";
print <<<ENDLINE
</select></td>
</tr>
</table>
...
<input type="button" name="bn2" value="Contacts" class="customer_details_submit" id="contact_button" onclick="open_contact_list();"> 
...
<div style="width: 1200px; margin-top: 15px;"><div class="customer_details_title">Recent Activities</div></div><p>&nbsp;</p>
<div class="customer_details_block">
<table class="customer_details_leads">
<tr><td dir="rtl">תאריך</td><td>Scheduled For</td><td>Activity Type</td><td>Interested In</td><td>User</td><td>Summary</td><td>Results</td></tr>
...
$sth = $dbh->query("select * from assignments where customer_code = '$session[customer_code]' order by hitkash_unix desc limit 5"); while ($details = $sth->fetch())
{
	...
	if ($details[as_result] == "price_quote") { $as_result = "Price Quote"; }
	else if ($details[as_result] == "not_interested") { $as_result = "Not Interested"; }
	else if ($details[as_result] == "order") { $as_result = "Order"; }
	else if ($details[as_result] == "removed") { $as_result = "Removed"; }

	if ($details[as_main_results] == "no_answer") { $as_main_results = "No Answer"; }
	else if($details[as_main_results] == "busy") { $as_main_results = "Busy"; }
	else if ($details[as_main_results] == "postpone") { $as_main_results = "Postpone"; }
	else if ($details[as_main_results] == "update_lead") { $as_main_results = "Update Lead"; }
	
	print "<tr><td>$details[hitkash_date] $details[hitkash_time]</td><td>$details[bizua_date] $details[bizua_time]</td><td>$activity_type[value]</td><td>$interested_in[value]<td>$username[fullname]</td>
	<td>$details[free_text]</td><td>$as_result *** $as_main_results</tr>";
}
...
function update_details()
{
	...
	if(!$session[customer_code])
	{
		...
		email_1 = '$session[email_1]', birth_date = '$session[birth_date]', passport_number = '$session[passport_number]', passport_exp_date = '$session[passport_exp_date]', status = 'active'");
	}
	else
	{
