...
ENDLINE;
	...
	{		
		...
		as_type = \"assignment\", interested_in = \"$session[interested_in]\", media_source = \"$session[media_source]\", departure_date = \"$session[departure_date]\", return_date = \"$session[return_date]\",
...
else if($session[status] == "update_details")
{
	...
	file_this("bizua_hour: $bizua_hour, bizua_minute: $bizua_minute, bizua_time: $bizua_time", "ul1");
	$bizua_unix = get_unixtime_from_mysql($bizua_date, $bizua_time);
	$activity = secure_numbers($session[activity]); if(!$activity) { $sth = $dbh->query("select code from table_details where is_default = 'yes' and table_code = '3'"); $activity = $sth->fetch(); 
		$activity = $activity[code]; }
	$interested_in = secure_numbers($session[interested_in]); if(!$interested_in) { $sth = $dbh->query("select code from table_details where is_default = 'yes' and table_code = '4'"); $interested_in = $sth->fetch();
		$interested_in = $interested_in[code]; }
	$free_text = secure_string($session[free_text]);
	$free_text = addslashes($free_text);
	$our_time = time();
	if ($our_time > $bizua_unix)
...
ENDLINE;
	...
	{		
		...
		interested_in = \"$interested_in]\", media_source = \"$session[media_source]\", departure_date = \"$session[departure_date]\", return_date = \"$session[return_date]\",
