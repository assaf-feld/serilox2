...
ENDLINE;
		if ($passport_line)
		{
			...
			{
				$("#confirm_update_div_1a", window.parent.document).fadeIn(300).delay(800).fadeOut(300);
			})
ENDLINE;
		}
		else
...
ENDLINE;
	...
	else
	{
		...
		$(document).ready(function()
		{
			$("#error_message_div_1a", window.parent.document).text("$error_message");
			$("#error_message_div_1a", window.parent.document).fadeIn(300).delay(800).fadeOut(300);
...
else if($session[customer_code])
{
	...
	if ($phone_1) { $phone_check_line .= " phone_1 regexp '$phone_1' or phone_2 regexp '$phone_1' "; }
	if ($phone_2) { $phone_check_line .= " phone_1 regexp '$phone_2' or phone_2 regexp '$phone_2' "; }
	if (!$phone_1 and !$phone_2) { $phone_check_line .= " 1 = 1 "; }
	$phone_check_line .= " ) ";
	file_this($phone_check_line, "uc3");
	$sth = $dbh->query($phone_check_line); $check_phone = $sth->fetch();
	if ($check_phone)
